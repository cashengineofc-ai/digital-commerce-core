...
async function loadBumps(supabase: any, source: Source) {
  const configured = (Array.isArray(source.checkout.produtos_config) ? source.checkout.produtos_config : [])
    .filter((item: unknown): item is Record<string, any> => Boolean(item && typeof item === "object"))
    .filter((item: Record<string, any>) => item.tipo === "order_bump" || item.type === "order_bump" || item.order_bump === true)
    .map((item: Record<string, any>) => ({
      id: String(item.id ?? item.bump_id ?? item.produto_id ?? item.product_id ?? ""),
      productId: String(item.produto_id ?? item.product_id ?? ""),
    }))
    .filter((item: { id: string; productId: string }) => item.id && isUuid(item.productId));
  if (!configured.length) return [];
  const productIds = [...new Set(configured.map((item: { productId: string }) => item.productId))];
  const productsResult = await supabase
    .from("produtos")
    .select("id,nome,descricao_curta,imagem_principal_url,preco,preco_promocional,promocao_inicio,promocao_fim,status,deleted_at,gerencia_estoque,estoque,estoque_reservado")
    .in("id", productIds)
  ...
  return configured.flatMap((item: { id: string; productId: string }) => {
    const product = products.get(item.productId) as Record<string, any> | undefined;
    ...
    return [{
      ...
      description: (product.descricao_curta ?? null) as string | null,
      image_url: (product.imagem_principal_url ?? null) as string | null,
      amount: activePromotionPrice(product),
    }];
  });
}
...
Deno.serve(async (request) => {
  ...
  const bumpsAmount = Number(bumpSnapshot.reduce((sum, bump) => sum + Number(bump.amount), 0).toFixed(2));
  const totalAmount = Number((baseAmount + bumpsAmount).toFixed(2));
  const metadata = {
    checkout_source: source.link ? "payment_link" : "checkout",
    public_checkout: true,
    order_bumps: bumpSnapshot,
    total_components: { base_amount: baseAmount, order_bumps_amount: bumpsAmount, total: totalAmount },
  };
  let existing = await supabase
    ...
    .maybeSingle();
  if (existing.error) return jsonResponse({ error: "idempotency_lookup_failed" }, 500);
  if (!existing.data) {
    ...
    const inserted = await supabase.from("transacoes").insert({
      ...
      codigo_externo: source.link?.codigo_unico ?? source.checkout.slug,
      ...
      metadata,
    }).select("id,pedido_id,status,status_detalhe_provedor,id_transacao_gateway,pix_copia_cola,pix_qrcode,pix_modo,pix_recebedor_nome,pix_recebedor_cidade,payload_provedor,valor_bruto,afiliado_id,link_afiliado_id").single();
    if (inserted.error?.code === "23505") {
      existing = await supabase.from("transacoes").select("id,pedido_id,status,status_detalhe_provedor,id_transacao_gateway,pix_copia_cola,pix_qrcode,pix_modo,pix_recebedor_nome,pix_recebedor_cidade,payload_provedor,valor_bruto,afiliado_id,link_afiliado_id").eq("empresa_id", source.empresaId).eq("idempotency_key", idempotencyKey).single();
    } else {
      existing = inserted;
    }
    if (existing.error || !existing.data) return jsonResponse({ error: "transaction_create_failed" }, 500);
  }

  const tx = existing.data;
  const persistedTotal = Number(tx.valor_bruto);
  const orderId = tx.pedido_id ?? undefined;

  if (provider === "pix_chave") {
    if (tx.pix_copia_cola && tx.pix_qrcode) {
      return jsonResponse({ ok: true, duplicate: true, order_id: orderId, transaction_id: tx.id, payment_id: tx.id, status: tx.status, status_detail: tx.status_detalhe_provedor, amount: persistedTotal, receiver_name: tx.pix_recebedor_nome, receiver_city: tx.pix_recebedor_cidade, manual_confirmation: true, message: "Aguardando conferência", pix: { qr_code: tx.pix_copia_cola, qr_code_base64: tx.pix_qrcode, ticket_url: null }, success_url: source.link?.url_redirecionamento_sucesso ?? source.checkout.url_sucesso ?? null });
    }
  ...
  if (tx.id_transacao_gateway) {
    ...
    return jsonResponse({ ok: true, duplicate: true, order_id: orderId, transaction_id: tx.id, payment_id: tx.id_transacao_gateway, status: tx.status, status_detail: tx.status_detalhe_provedor, amount: persistedTotal, manual_confirmation: false, message: null, pix: pix ? { qr_code: pix.qr_code ?? null, qr_code_base64: pix.qr_code_base64 ?? null, ticket_url: pix.ticket_url ?? null } : null, success_url: source.link?.url_redirecionamento_sucesso ?? source.checkout.url_sucesso ?? null });
  }
  ...
  return jsonResponse({ ok: true, order_id: orderId, transaction_id: tx.id, payment_id: String(payment.id), status: mapped, status_detail: payment.status_detail ?? null, amount: persistedTotal, manual_confirmation: false, message: null, pix: pixData ? { qr_code: pixData.qr_code ?? null, qr_code_base64: pixData.qr_code_base64 ?? null, ticket_url: pixData.ticket_url ?? null } : null, success_url: source.link?.url_redirecionamento_sucesso ?? source.checkout.url_sucesso ?? null });
});
