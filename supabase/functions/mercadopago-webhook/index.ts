...
Deno.serve(async (request) => {
  ...
  const bodyDataId =
    notification.data?.id != null ? String(notification.data.id) : null;
  ...
  if (eventId) {
    const { error } = await supabase
      .from("webhook_events")
      .update({ status: "processing", attempts, payload: notification, error: null })
      .eq("id", eventId);
    if (error) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      await finishEvent("failed", {
        error: `mercadopago_${response.status}:${errorBody.slice(0, 500)}`,
      });
      return jsonResponse(
    ...
    await finishEvent("failed", {
      error: error instanceof Error ? error.message : "payment_lookup_failed",
    });
  ...
  const transactionColumns =
    "id,valor_bruto,id_transacao_gateway,payload_provedor,provedor_pagamento";
  ...
  if (!transaction && externalReference) {
    const byReference = await supabase
      ...
      .select(transactionColumns)
      .or(
        `id.eq.${externalReference},pedido_numero.eq.${externalReference},codigo_externo.eq.${externalReference}`,
      )
      .limit(1)
      .maybeSingle();
  ...
  const receivedCollector = payment.collector_id;
  ...
  const methodMatches = payment.payment_method_id === "pix";
  const gatewayMatches =
    !transaction.id_transacao_gateway ||
    transaction.id_transacao_gateway === paymentId;
  const referenceMatches =
    externalReference === transaction.id || metadataTransactionId === transaction.id;
  const amountMatches =
    amount !== undefined &&
    Math.abs(amount - Number(transaction.valor_bruto)) <= 0.01;
  const collectorMatches =
    originalCollector != null &&
    receivedCollector != null &&
    String(originalCollector) === String(receivedCollector);
  ...
  const processingFee = Array.isArray(payment.fee_details)
    ? payment.fee_details.reduce(
        (sum, fee) => sum + (numberOrUndefined(fee.amount) ?? 0),
        0,
      )
    : undefined;
  ...
  if (
    numberOrUndefined(payment.transaction_details?.net_received_amount) !==
    undefined
  ) {
