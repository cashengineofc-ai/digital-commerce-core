-- Forward backfill of settings RPCs, with serialized credential rotation.
...
BEGIN
  ...
  PERFORM pg_advisory_xact_lock(hashtextextended(v_empresa::text||':'||p_provider,0));
  ...
  v_secret_id:=vault.create_secret(
    p_secret,
    'cash_engine_'||p_provider||'_'||v_empresa::text||'_'||gen_random_uuid()::text,
...
-- Credentials and connection status are changed only through authorized RPCs.
REVOKE ALL ON public.integracoes,public.integracoes_logs FROM PUBLIC,anon,authenticated;
