-- Forward repair/backfill for databases where the team migrations were never installed.
-- No accounts, invitations, payments or balances are created by this migration.
ALTER TABLE public.invites ADD COLUMN IF NOT EXISTS metadata jsonb NOT NULL DEFAULT '{}'::jsonb;
ALTER TABLE public.invites ADD COLUMN IF NOT EXISTS revogado_em timestamptz;
ALTER TABLE public.invites ADD COLUMN IF NOT EXISTS revogado_por uuid REFERENCES public.profiles(id);

CREATE OR REPLACE FUNCTION public.fn_is_admin_global()
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public AS $$
  SELECT EXISTS(SELECT 1 FROM public.profiles WHERE id=auth.uid()
    AND is_admin_global AND status='ativo' AND deleted_at IS NULL);
$$;
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public AS $$
  SELECT EXISTS(SELECT 1 FROM public.profiles p JOIN public.empresas e ON e.id=p.empresa_id
    WHERE p.id=auth.uid() AND p.empresa_id=coalesce(p_empresa_id,public.current_empresa_id())
      AND p.is_owner AND p.status='ativo' AND p.deleted_at IS NULL
      AND e.status='ativo' AND e.deleted_at IS NULL);
$$;
REVOKE ALL ON FUNCTION public.fn_is_admin_global(),public.is_admin_global(),public.fn_is_empresa_owner(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.fn_is_admin_global(),public.is_admin_global(),public.fn_is_empresa_owner(uuid) TO anon,authenticated,service_role;
...
DROP POLICY IF EXISTS profile_company_context_self_read ON public.profile_company_context;
...
AS $$
  SELECT auth.uid() IS NOT NULL
    ...
    AND EXISTS (SELECT 1 FROM public.empresas e WHERE e.id=p_empresa_id AND e.status='ativo' AND e.deleted_at IS NULL)
    AND (
      EXISTS(
        ...
        WHERE p.id=auth.uid()
          ...
          AND (p.is_owner OR NOT EXISTS (
            SELECT 1 FROM public.equipe_membros revoked
            WHERE revoked.profile_id=p.id AND revoked.empresa_id=p_empresa_id
              AND (revoked.status<>'ativo' OR revoked.deleted_at IS NOT NULL)
          ))
      )
      OR EXISTS(
...
AS $$
  SELECT CASE
    ...
    ELSE (
      ...
      WHERE p.id=auth.uid() AND public.fn_usuario_tem_empresa(p.empresa_id)
    )
  END;
$$;
...
BEGIN
  ...
  WITH memberships AS (
    ...
    FROM public.profiles p
    WHERE p.id=auth.uid()
      AND public.fn_usuario_tem_empresa(p.empresa_id)
    ...
    WHERE em.profile_id=auth.uid()
      AND em.status='ativo'
      AND em.deleted_at IS NULL
      AND public.fn_usuario_tem_empresa(em.empresa_id)
  )
  SELECT
    e.id,
...
BEGIN
  ...
  IF p_expira_dias IS NULL OR p_expira_dias<1 OR p_expira_dias>30 THEN RAISE EXCEPTION 'team_invite_expiration_invalid'; END IF;
  ...
  PERFORM public.fn_assert_role_delegavel(p_role_id);
...
BEGIN
  ...
  IF NOT FOUND
     OR v_i.token_hash IS NULL
     OR v_i.token_hash IS DISTINCT FROM encode(digest(trim(coalesce(p_token,'')),'sha256'),'hex') THEN
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.profiles WHERE id=auth.uid() AND status='ativo' AND deleted_at IS NULL)
     OR NOT EXISTS (SELECT 1 FROM public.empresas WHERE id=v_i.empresa_id AND status='ativo' AND deleted_at IS NULL) THEN
    RAISE EXCEPTION 'inactive_account_or_company';
  END IF;
  IF v_i.role_id IS NULL OR NOT EXISTS (SELECT 1 FROM public.roles WHERE id=v_i.role_id AND deleted_at IS NULL AND (empresa_id=v_i.empresa_id OR empresa_id IS NULL)) THEN
...
BEGIN
  ...
  SELECT
    ...
    pr.role_id,r.nome::text,(p.is_owner AND p.empresa_id=v_empresa OR m.owner_flag)
  FROM members m
  ...
  ORDER BY (p.is_owner AND p.empresa_id=v_empresa OR m.owner_flag) DESC,p.nome_completo;
END;
$$;
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.profiles WHERE id=p_profile_id AND empresa_id=v_empresa AND is_owner) THEN RAISE EXCEPTION 'cannot_change_owner_role'; END IF;
  PERFORM public.fn_assert_role_delegavel(p_role_id);
...
BEGIN
  ...
  PERFORM public.fn_assert_permissions_delegaveis(p_permission_ids);
...
-- All authorization entry points must agree on the active, still-authorized company.
CREATE OR REPLACE FUNCTION public.fn_get_empresa_usuario()
RETURNS uuid LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public AS $$
  SELECT public.current_empresa_id();
$$;
REVOKE ALL ON FUNCTION public.fn_get_empresa_usuario() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.fn_get_empresa_usuario() TO authenticated,service_role;

CREATE OR REPLACE FUNCTION public.fn_tem_permissao(p_modulo varchar,p_recurso varchar,p_acao public.tipo_operacao)
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path=public AS $$
DECLARE v_empresa uuid:=public.current_empresa_id();
BEGIN
  IF auth.uid() IS NULL THEN RETURN false; END IF;
  IF public.fn_is_admin_global() THEN RETURN true; END IF;
  IF lower(coalesce(p_modulo,'')) IN ('developers','admin','admin_global') THEN RETURN false; END IF;
  IF v_empresa IS NULL OR NOT public.fn_usuario_tem_empresa(v_empresa) THEN RETURN false; END IF;
  IF public.fn_is_empresa_owner(v_empresa) THEN RETURN true; END IF;
  RETURN EXISTS (
    SELECT 1 FROM public.profile_roles pr JOIN public.roles r ON r.id=pr.role_id
    WHERE pr.profile_id=auth.uid() AND pr.empresa_id=v_empresa AND r.deleted_at IS NULL
      AND (r.empresa_id=v_empresa OR r.empresa_id IS NULL)
      AND (pr.expira_em IS NULL OR pr.expira_em>now())
      AND (r.is_admin OR EXISTS (
        SELECT 1 FROM public.role_permissions rp JOIN public.permissions p ON p.id=rp.permission_id
        WHERE rp.role_id=r.id AND p.modulo=p_modulo AND p.recurso=p_recurso AND p.acao=p_acao
      ))
  );
END;
$$;
REVOKE ALL ON FUNCTION public.fn_tem_permissao(varchar,varchar,public.tipo_operacao) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION public.fn_tem_permissao(varchar,varchar,public.tipo_operacao) TO authenticated,service_role;

CREATE OR REPLACE FUNCTION public.fn_assert_permissions_delegaveis(p_ids uuid[])
RETURNS void LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path=public AS $$
BEGIN
  IF auth.uid() IS NULL OR public.current_empresa_id() IS NULL THEN RAISE EXCEPTION 'not_authenticated'; END IF;
  IF EXISTS (SELECT 1 FROM unnest(coalesce(p_ids,ARRAY[]::uuid[])) x
    WHERE x IS NULL OR NOT EXISTS(SELECT 1 FROM public.permissions WHERE id=x)) THEN
    RAISE EXCEPTION 'permission_not_found';
  END IF;
  IF public.fn_is_admin_global() OR public.fn_is_empresa_owner(public.current_empresa_id()) THEN RETURN; END IF;
  IF EXISTS (SELECT 1 FROM public.permissions p WHERE p.id=ANY(coalesce(p_ids,ARRAY[]::uuid[]))
    AND NOT public.fn_tem_permissao(p.modulo,p.recurso,p.acao)) THEN
    RAISE EXCEPTION 'cannot_delegate_unowned_permission';
  END IF;
END;
$$;

CREATE OR REPLACE FUNCTION public.fn_assert_role_delegavel(p_role_id uuid)
RETURNS void LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path=public AS $$
DECLARE r public.roles%ROWTYPE; v_ids uuid[];
BEGIN
  IF auth.uid() IS NULL OR public.current_empresa_id() IS NULL THEN RAISE EXCEPTION 'not_authenticated'; END IF;
  SELECT * INTO r FROM public.roles WHERE id=p_role_id AND deleted_at IS NULL
    AND (empresa_id=public.current_empresa_id() OR empresa_id IS NULL);
  IF NOT FOUND THEN RAISE EXCEPTION 'role_not_available'; END IF;
  IF r.is_admin AND NOT (public.fn_is_admin_global() OR public.fn_is_empresa_owner(public.current_empresa_id())) THEN
    RAISE EXCEPTION 'owner_required_for_admin_role';
  END IF;
  SELECT array_agg(permission_id) INTO v_ids FROM public.role_permissions WHERE role_id=r.id;
  PERFORM public.fn_assert_permissions_delegaveis(v_ids);
END;
$$;
REVOKE ALL ON FUNCTION public.fn_assert_permissions_delegaveis(uuid[]),public.fn_assert_role_delegavel(uuid) FROM PUBLIC,anon,authenticated;

-- Browser writes must pass through the audited RPCs above; permissive legacy
-- tenant policies must not allow assigning roles or fabricating an invitation.
REVOKE INSERT,UPDATE,DELETE,TRUNCATE,REFERENCES,TRIGGER ON
  public.roles,public.role_permissions,public.profile_roles,public.equipe_membros,public.invites
